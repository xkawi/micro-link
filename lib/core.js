...
const jwtSecret = 'tHis1sLean54cretH54'; // change accordingly
const tokenParamKey = 'token';

/**
 * generate dynamic deeplink
 */
const generateDynamicLink = async (data) => {
  const token = jwt.sign(data, jwtSecret);
  const isProd = 'production' === process.env.NODE_ENV;
  const leanHost = isProd ? process.env.NOW_URL : 'localhost:3000'
  return `${leanHost}?${tokenParamKey}=${token}`;
}
